// // src/modales/ModalSeleccionChasis.tsx
// //Este modal pertenece a src/PuntoVenta.tsx
// import { useState, useEffect } from 'react';
// import { X, CheckCircle, Bike, AlertCircle } from 'lucide-react';
// // ✨ IMPORTAMOS TU SERVICIO
// // import { VentasService, StockVenta, ChasisDisponible } from '../services/ventas_service';
// import { VentasService, StockVenta } from '../services/ventas_service';
...
// export default function ModalSeleccionChasis({ isOpen, onClose, producto, onConfirmar }: Props) {
...
//     // ✨ CONSULTA REAL A SQLITE VÍA TAURI
//     useEffect(() => {
...
//                 try {
//                     const disponibles = await VentasService.obtenerChasisDisponibles(producto.lote_id);
//                     setChasisDisponibles(disponibles);
//                 } catch (error) {
//                     console.error("Error al buscar chasis:", error);
//                 } finally {
//                     setCargando(false);
//                 }
//             }
...
//                     </div>
//                     <button onClick={onClose} className="text-amber-100 hover:text-white hover:bg-amber-600 p-1 rounded-md transition-colors">
//                         <X size={20} />
//                     </button>
//                 </div>
...
//                             <p className="text-xs font-bold text-slate-400 mb-2 uppercase">Chasis Disponibles en inventario ({chasisDisponibles.length})</p>
...
//                                     <AlertCircle size={20} />
//                                     <p className="text-sm font-medium">No hay chasis disponibles para este modelo.</p>
//                                 </div>
//                             )}
...
//                                 <label
//                                     key={chasis.id}
//                                     className={`flex items-center p-3 border-2 rounded-xl cursor-pointer transition-all ${chasisSeleccionadoId === chasis.id ? 'border-amber-500 bg-amber-50' : 'border-slate-200 bg-white hover:border-amber-300'}`}
//                                 >
//                                     <input
//                                         type="radio"
//                                         name="chasis_select"
...
//                                         <p className="font-mono font-bold text-sm text-slate-800">VIN: {chasis.numero_chasis}</p>
//                                         <p className="font-mono text-xs text-slate-500">Motor: {chasis.numero_motor}</p>
//                                     </div>
//                                     <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${chasisSeleccionadoId === chasis.id ? 'border-amber-500 bg-amber-500' : 'border-slate-300'}`}>
//                                         {chasisSeleccionadoId === chasis.id && <div className="w-2 h-2 bg-white rounded-full" />}
//                                     </div>
//                                 </label>
//                             ))}
//                         </div>
