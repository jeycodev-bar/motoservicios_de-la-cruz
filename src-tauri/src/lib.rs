...
pub fn run() {
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            tauri::async_runtime::block_on(async {
                match db::inicializar_base_datos(&app_dir).await {
                    Ok(pool) => {
                        app.manage(pool);
                    }
                    Err(err_msg) => {
                        // Guardar el error exacto en un archivo para depurar en la PC del cliente
                        let log_path = app_dir.join("error_log.txt");
                        if let Ok(mut file) =
                            OpenOptions::new().create(true).append(true).open(&log_path)
                        {
                            let _ = writeln!(file, "Error al iniciar BD: {}", err_msg);
                        }
                        panic!("{}", err_msg); // Abortar con el mensaje real de error
                    }
                }
            });
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            taller::obtener_ordenes_activas,
            taller::archivar_ordenes_viejas,
            taller::obtener_catalogo_repuestos,
            taller::obtener_detalles_orden,
            taller::obtener_historial_paginado_taller,
            maestros::crear_categoria_segura,
            maestros::eliminar_categoria_segura,
            maestros::crear_marca_segura,
            maestros::eliminar_marca_segura,
            maestros::obtener_categorias,
            maestros::obtener_marcas,
            maestros::obtener_marcas_por_categoria,
            inventario::registrar_ingreso_seguro,
            inventario::obtener_inventario_reciente,
            inventario::obtener_stock_bodega,
            inventario::agregar_stock_existente_seguro,
            inventario::obtener_stock_bodega_agrupado,
            catalogo::crear_producto_seguro,
            catalogo::actualizar_producto_seguro,
            catalogo::eliminar_producto_seguro,
            catalogo::obtener_productos_paginados,
            ...
            clientes::guardar_cliente_seguro,
            clientes::actualizar_cliente_seguro,
            clientes::buscar_cliente_por_documento,
            clientes::obtener_clientes_paginados,
            clientes::buscar_clientes_rapido,
            auth::login_seguro,
            auth::crear_usuario_seguro,
            auth::actualizar_usuario_seguro,
            auth::cambiar_estado_usuario_seguro,
